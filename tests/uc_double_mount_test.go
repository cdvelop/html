...
func (c *mountCountComp) String() string { return c.Element.String() }
...
func TestRender_OnMount_CalledOnce(t *testing.T) {
	...
	child := &mountCountComp{Element: Div().ID("mount-child")}

	container := Div(child).ID("mount-container")
...
func TestRender_OnMount_MultipleChildren(t *testing.T) {
	...
	a := &mountCountComp{Element: Div().ID("mc-a")}
	b := &mountCountComp{Element: Div().ID("mc-b")}
	c := &mountCountComp{Element: Div().ID("mc-c")}

	container := Div(a, b, c).ID("mc-container")
...
func TestAppend_OnMount_CalledOnce(t *testing.T) {
	...
	child := &mountCountComp{Element: Div().ID("append-child")}
	container := Div(child).ID("append-container")
