...
func (c *SelfUpdater) Render() *dom.Element {
	return Div(
		...
		Div().
			...
			Add(Div().ID(c.GetID()+"-opt-a").Attr("data-id", "a").Text("Option A")),
		P().ID(c.GetID()+"-result").Text(c.selected),
	)
}
...
func (c *SSChild) Render() *dom.Element {
	return Div(
		...
		Div().
			...
			Add(
				Div().ID(c.GetID()+"-opt-a").Attr("data-id", "a").Text("Option A"),
				Div().ID(c.GetID()+"-opt-b").Attr("data-id", "b").Text("Option B"),
			),
	)
}
